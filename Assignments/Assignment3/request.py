...
from pokeretriever.http_request import HttpRequest
...
class RequestDataHandler(BaseRequestHandler):
    def handle_request(self, request: Request, **kwargs):
        ...
        response = loop.run_until_complete(HttpRequest.process_requests(kwargs.get("ids")))
        kwargs["response"] = response
        print(response[0]["abilities"])
